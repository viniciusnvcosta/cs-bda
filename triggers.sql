...
BEGIN
    ...
    SELECT MIN(Dia) INTO month_start FROM Registro WHERE Medidor_ID = NEW.Medidor_ID;
    SELECT MAX(Dia) INTO month_end FROM Registro WHERE Medidor_ID = NEW.Medidor_ID;
    SELECT (MAX(Diario) - MIN(Diario)) * 0.75 INTO iqr FROM Lancamento
        JOIN Registro ON Lancamento.Medidor_ID = Registro.Medidor_ID AND Lancamento.Dia = Registro.Dia
